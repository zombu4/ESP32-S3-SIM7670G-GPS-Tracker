...
static bool start_all_tasks_impl(void)
{
 ...
 // MQTT Communication Task - Core 0, Normal Priority (Large stack for MQTT AT commands)
...
void mqtt_communication_task(void* params)
{
 ...
 // Initialize MQTT with configuration from GPS tracker
 const mqtt_config_t* mqtt_config = gps_tracker_get_mqtt_config();
 if (mqtt && !mqtt->init(mqtt_config)) {
...
void system_watchdog_task(void* params)
{
 ...
 while (g_task_manager.tasks_running) {
 // Feed main watchdog only if successfully registered
 if (watchdog_registered) {
 esp_task_wdt_reset();
 }
...
{
 ESP_LOGI(TAG, " [Core 1] GPS Data Collection Task started");
 ...
 ESP_LOGW(TAG, " GPS polling skipped - UART busy with AT commands");
 }
 ...
 // Break delay into smaller chunks to feed watchdog regularly (every 5s)
 for (int i = 0; i < 3; i++) {
 vTaskDelay(pdMS_TO_TICKS(5000)); // 5 second chunks
 
 // Feed watchdog during delay only if successfully registered
 if (watchdog_registered && g_task_manager.tasks_running) {
 esp_task_wdt_reset();
 }
 }
 }
