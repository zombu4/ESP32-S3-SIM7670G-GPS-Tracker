...
static bool send_mqtt_at_command(const char* command, const char* expected, int timeout_ms)
{
 ...
 // FIXED: Check for both "OK" and "YES" responses (SIM7670G variations)
 bool found_expected = false;
 if (strcmp(expected, "OK") == 0) {
 // For "OK" expectation, accept both "OK" and "YES"
 found_expected = (strstr(response.response, "OK") != NULL || strstr(response.response, "YES") != NULL);
 if (current_config.debug_mqtt) {
 ESP_LOGI(TAG, "[MQTT] Expected 'OK', found: %s", found_expected ? "OK/YES" : "NONE");
 }
 } else {
 // For other expectations, use exact match
